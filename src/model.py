"""
DARNet model architecture.

Source: official baseline repo (https://github.com/fchest/DARNet),
copied as-is so we have a stable local copy without depending on
re-cloning the repo each time.

Expects input x of shape (batch_size, channels, seq_len) -- i.e. the
same (32, 128) per-sample shape AADDataset already produces.

Usage:
    config = {'Data_shape': (N, 32, 128)}   # N is unused inside the model,
                                              # only [1]=channels, [2]=seq_len matter
    model = DARNet(config)
    out = model(batch_x)   # batch_x shape: (batch_size, 32, 128)
    # out shape: (batch_size, 2)  -- raw logits for [left, right]
"""
import math
import torch
